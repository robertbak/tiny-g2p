...
The same model, built three ways: one static binary, one `.wasm`, and one ONNX
graph, all from the same core. The core has **no native dependencies** — the
weights are `include_bytes!`d and the blob is parsed by hand — so `cargo build
--release` produces a single executable and `wasm-pack` a single `.wasm`.

The model is trained by its [Python sibling](https://github.com/robertbak/tiny_g2p),
which is where the weights come from; this repository is everything needed to run
them, and it is self-contained once they are here.
...
cargo test --release --workspace           # 33 unit + 2 examples + 5 parity + 1 doc test
...
The binary embeds `core/weights/tiny_g2p.bin`, so the blob has to exist before
the crate can be built at all. It is **exported by the training repository**, not
by this one — the model is trained there, and the export is a Python step:

```bash
# in a checkout of github.com/robertbak/tiny_g2p, beside this one
make export              # RUST_DIR ?= ../tiny-g2p, so it writes into this repo
```

That writes `core/weights/tiny_g2p.bin` and `tiny_g2p.onnx` here, and the two
repositories are expected to sit side by side for exactly this reason. The blob
is versioned (`TG2P`/1) and self-describing; a mismatched version is rejected
rather than misread, and the training side holds the reference writer with a
test on each side.

`data/test_gold.tsv` is the same arrangement: generated there, committed here,
because `tinyg2p eval` and `tinyg2p miss-lexicon --gold` need it and this
repository should be able to run them without the training checkout.

## Layout

| path | what |
|---|---|
| `core/src/blob.rs` | the blob reader (mirror of the training side's writer) |
| `core/src/model.rs` | float and int8 forward pass |
| `core/src/text.rs` | Polish NFC-lite normalisation, vocabularies |
| `core/src/phones.rs` | `restore`: the nasal merge split back |
| `core/src/canon.rs` | the MFA-side canonicalization used for scoring |
| `core/src/exceptions.rs` | asserted and suggested dictionaries, acronym table, initialism rule |
| `core/src/gold.rs` | gold TSV reader, the scorer, and the notation comparison |
| `core/tests/parity.rs` | bit-exactness against `golden_predictions.json` |
| `core/weights/` | generated by the training repo: the blob, and the ONNX graph |
| `src/main.rs` | the CLI: `predict`, `eval`, `miss-lexicon`, `info` |
| `wasm/src/lib.rs` | the browser surface |
| `smoke.mjs` | the wasm build, exercised under node |
| `examples/` | eight real utterances, their phones, an asserted dictionary |
| `data/test_gold.tsv` | the held-out split, generated by the training repo |
...
they are committed here and embedded in the crates above, so using them
redistributes something derived from CC BY 4.0 data. The MIT licence on the code
does not say so, which is why it is said here.
